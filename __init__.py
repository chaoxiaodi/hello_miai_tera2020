...
class xiaomi_tts:
    ...
    def request_app_setup(self, image_name):
        ...
        def fitbit_configuration_callback(callback_data):
            ...
            if self.Service_Token is not None and self.deviceIds is not None:
                self.hass.components.configurator.request_done(self._CONFIGURING.pop("MIAI"))
    ...
    def _serviceLoginAuth2(self, captCode=None):
        ...
        try:
            ...
            r = self._request.post(url, headers=self._headers, data=auth_post_data, timeout=3,
    ...
    def text_to_speech(self, text, num=0):
        try:
            url = "https://api.mina.mi.com/remote/ubus?" \
                  "deviceId={}&message=%7B%22text%22%3A%22{}%22%7D&method=text_to_speech&path=mibrain&requestId={}"\
                .format(self.deviceIds[num]['deviceID'], parse.quote(text),
                        ''.join(random.sample(string.ascii_letters + string.digits, 30)))
            r = self._request.post(url,headers={'Cookie':self.Service_Token}, timeout=10,verify=False)
    ...
    def player_set_volume(self, volume, num=0):
        if volume > 100:
            volume = 100
        elif volume < 0:
            volume = 0
        try:
            url = "https://api.mina.mi.com/remote/ubus?" \
                  "deviceId={}&message=%7b%22volume%22%3a{}%2c%22media%22%3a%22app_ios%22%7d&" \
                  "method=player_set_volume&path=mediaplayer&requestId={}"\
                .format(self.deviceIds[num]['deviceID'],
                        int(volume),
                        ''.join(random.sample(string.ascii_letters + string.digits, 30)))
            r = self._request.post(url,headers={'Cookie':self.Service_Token}, timeout=10, verify=False)
            if json.loads(r.text)['message'] == 'Success':
    ...
    def player_play_operation(self,operation, num=0):

        try:
            url = "https://api.mina.mi.com/remote/ubus?" \
                  "deviceId={}&message=%7b%22action%22%3a%22{}%22%2c%22media%22%3a%22app_ios%22%7d&" \
                  "method=player_play_operation&path=mediaplayer&requestId={}"\
                .format(self.deviceIds[num]['deviceID'],
                        operation,
                        ''.join(random.sample(string.ascii_letters + string.digits, 30)))
            r = self._request.post(url, headers={'Cookie':self.Service_Token}, timeout=10, verify=False)
...
SERVICE_SCHEMA_FOR_PLAY_OPERATION = vol.Schema({ vol.Optional(CONF_TO_NUM): cv.string })
...
def setup(hass, config):
    ...
    def listen_to_msg():
        while not Event().isSet():
            if client.Login_result:
                if len(msg_queue) > 0:
                    send_finish = client.text_to_speech(msg_queue[0]['msg'], int(msg_queue[0]['to_num']))
                    if send_finish:
                        try:
    ...
    def send_message(call):
        ...
        if client.Service_Token_Cookie is None or client.deviceIds_miai is None:
            ...
            if not client.text_to_speech(message, int(to_num)):
                client._LoginByPassord()
                client.text_to_speech(message, int(to_num))
    ...
    def player_set_volume(call):
        ...
        if client.Service_Token_Cookie is None or client.deviceIds_miai is None:
            ...
            if not client.player_set_volume(int(vol), int(to_num)):
                client._LoginByPassord()
                client.player_set_volume(int(vol), int(to_num))
    ...
    def play_operation(call):
        ...
        if client.Service_Token_Cookie is None or client.deviceIds_miai is None:
            ...
            if not client.player_play_operation('play', int(to_num)):
                client._LoginByPassord()
                client.player_play_operation('play', int(to_num))
    ...
    def pause_operation(call):
        ...
        if client.Service_Token_Cookie is None or client.deviceIds_miai is None:
            ...
            if not client.player_play_operation('pause', int(to_num)):
                client._LoginByPassord()
                client.player_play_operation('pause', int(to_num))
    ...
    hass.bus.listen_once("homeassistant_stop", _stop_listener)
    hass.services.register(DOMAIN, 'force_send', send_message, schema=SERVICE_SCHEMA)
    hass.services.register(DOMAIN, 'add2MsgQueue', add_msg2queue, schema=SERVICE_SCHEMA_FOR_QUEUE)
    hass.services.register(DOMAIN, 'set_vol', player_set_volume, schema=SERVICE_SCHEMA_FOR_SET_VOLUME)
    hass.services.register(DOMAIN, 'play', play_operation, schema=SERVICE_SCHEMA_FOR_PLAY_OPERATION)
    hass.services.register(DOMAIN, 'pause', pause_operation, schema=SERVICE_SCHEMA_FOR_PLAY_OPERATION)
